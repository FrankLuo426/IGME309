...
eBTX_OUTPUT FileReader::SaveBinaryFile(String a_sFileName)
{
...
eBTX_OUTPUT FileReader::ReadBinaryFile(String a_sFileName)
{
...
eBTX_OUTPUT FileReader::ReadFile(String a_sFileName)
{
	//if(m_pFile != nullptr)
	//	CloseFile();
...
eBTX_OUTPUT FileReader::SaveFile(String a_sFileName)
{
	if (m_pFile != nullptr)
		CloseFile();
...
eBTX_OUTPUT FileReader::RemoveTabs(String* a_sInput)
{
	uint nLenght = (*a_sInput).length();
	if (nLenght == 0)
		return eBTX_OUTPUT::OUT_ERR_CONTEXT;
...
eBTX_OUTPUT FileReader::ReadNextLine(bool bSkipComments)
{
	//if the file is not open return
	if (!m_iStream.is_open())
		return eBTX_OUTPUT::OUT_ERR_NOT_INITIALIZED;

	//if we are at the end of the file return
	if (m_iStream.eof())
		return eBTX_OUTPUT::OUT_DONE;
...
eBTX_OUTPUT FileReader::RemoveBlanks(String* a_sInput)
{
...
eBTX_OUTPUT FileReader::RemoveBlanks(char* a_szInput)
{
...
eBTX_OUTPUT FileReader::RemoveBlanks(char** zsInput)
{
